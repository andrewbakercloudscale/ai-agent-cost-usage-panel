...
- **The panel is told which session it is watching; it does not work it out.** A project directory holds every transcript that directory has ever produced, so "which of these is the conversation in the pane next to me" has no answer the panel can compute — two `claude` sessions open in one directory are indistinguishable by file. The answer is written down instead: `claude-panel-session-hook.sh` fires on Claude Code's `SessionStart` event and writes `<session-id>` to `~/.cache/claude-panel-pin/<project-dir>`, which the panel reads on every tick. Because the hook runs *inside* the session, it is authoritative rather than predictive — it covers `--resume`, `--continue`, GUI windows and IDE terminals, none of which the `~/.zshrc` `preexec` hook can pin in advance. The launcher writes the same file for the id it chose, so the pin still works with hooks disabled.
...
- **`Model: Unknown` and `no active Claude Code session found`, while every other figure is correct** — the panel has not been told which transcript is this pane's. Check `~/.cache/claude-panel-pin.log`: it records every pin adopted, ignored or abandoned. Confirm `~/.claude/settings.json` has the `SessionStart` hook (`jq '.hooks.SessionStart' ~/.claude/settings.json`) and that `~/.cache/claude-panel-pin/<your-project-dir>` exists and names your current session id. Re-running `bash deploy.sh claude` installs the hook; it takes effect on the *next* session start, not the current one.

...
Also installs a **cost-alert hook** (`claude-cost-alert-check.sh`, wired into Claude Code's `UserPromptSubmit` hook) that posts a warning *into the chat itself* — so it works over Remote Control too, not just locally — when the current session's cost crosses 2x (red) or 3x (purple) your 7-day average session cost, or when the panel failed to auto-launch for this window. There are two independent rules — a **per-session** one (this session vs your recent sessions) and a **per-day** one (today's total across every session, against a 3-sigma control limit). They read:

```
UserPromptSubmit says: 🟥 DAILY SPEND — $842.10 today, over your $786.95 3σ limit (mean $193.24 over 29 days)
...
- **The daily rule exists because the session rule structurally cannot see a bad day.** The session rule compares *one* session against the average session, so a day made of twenty ordinary sessions never trips it however much it totals — you can spend $150 across a day in silence. The daily rule is `mean + 3·sd` over the preceding days (sample sd, days with no activity simply absent rather than counted as zero), throttled once per calendar day and claimed with an atomic `mkdir` so that N open windows raise one alert rather than N.

- **But know what a 3-sigma limit is made of here, because the alert cannot tell you.** This data is heavily right-skewed and its sd is about the size of its mean — the 30 days to 2026-09-09 give mean $190.78, sd $194.93, so a limit of **$775.56** that exactly one day in that window exceeds. It is a genuine extreme-outlier alarm, not a busy-day one. And the limit moves with the *window's composition* as much as with behaviour: the same 30 days minus their first fortnight give mean $104.52, sd $86.44 — a limit of **$363.83**. As a heavy stretch rolls out of the window the alert silently becomes twice as sensitive, having been told nothing new. That is why `bash check-panel-status.sh` prints the live limit and the sample behind it, and why fewer than 7 prior days disables the rule outright and says so rather than acting on an sd computed from three days.
